...
- [x] 2026-09-19 — **Distribuição dinâmica de peso (Opção A, por barra — decisão do cliente)**: `editarMonte` agora grava peso editado como **peso real** (zera o estimado, RF-P02) e **dispara `reconciliarLote`** — o gatilho que faltava (antes só mover integral/venda reconciliavam; lotes sem `peso_total` continuam no-op); grade da **entrada** exibe o **peso estimado dinâmico** por célula (`peso_total ÷ barras marcadas × barras do monte`, recalcula enquanto preenche, laranja itálico + "est.") em vez de só "Xb"; estoque: patch otimista da edição marca `estimado: false` e hint explica a reconciliação; PRD RF-P02 + fluxo §20.5 atualizados (edição como gatilho de pesagem real)
- [x] 2026-09-19 — **Overlays separados: BottomSheet vs Popover** (sem bibliotecas externas): novo primitivo `<Popover>` (`ui.tsx`) — balão flutuante ancorado via prop `posicaoClassName`, `createPortal` no body, backdrop invisível (clique-fora fecha) + Esc, animação fade+scale própria (`globals.css .ios-pop`, propriedade `scale` compõe com o translate do posicionamento); **sino** migrado de BottomSheet→Popover (`fixed top-16 right-4 w-72 origin-top-right`, lista com `max-h-[60dvh]` scroll) — nasce no canto superior direito sem vazar da tela em mobile/desktop; **FAB "+"/Ações rápidas** migrado para `fixed bottom-24 left-1/2 -translate-x-1/2 w-64 origin-bottom` — flutua centralizado acima do botão; `BottomSheet` permanece para os formulários modais (ações do estoque, edição/comparação da contagem)
- [x] 2026-09-19 — **Padronização visual dos estados no estoque**: removida a cor da liga das bordas dos montes (estilo inline `borderColor: corHex` → borda neutra `border-[var(--border)]` em todos; a identificação por liga permanece só no badge do card do lote); **No setor** agora usa o mesmo roxo do card de resumo (`bg-[var(--roxo-soft)]`, sem mais opacity/dashed) e **o texto do bloco (peso, kg, barras) espelha o card** em `text-[var(--roxo)]` — antes caía no `--foreground`, quase branco no tema escuro; **Disponível** sem verde — neutro no card de resumo (cor padrão de texto) e nos montes; legenda realinhada ao novo padrão com item "No setor" incluído; Reservado âmbar e Parcial teal tracejado mantidos
- [x] 2026-09-19 — **Pesos estimados em números inteiros com compensação (opção A)**: distribuição proporcional por barra sem casas decimais — N-1 montes via `Math.round(peso × barras_monte / barras_totais)` com acumulador, e o **último monte fecha exato** com `peso − soma acumulada` (soma final sempre bate com o peso total, zero divergência de arredondamento); aplicado em `criarEntrada` (estimados do lote novo), `reconciliarLote` (reajuste dos não pesados — guarda `max(0, …)` quando não sobra peso) e na grade da entrada (`pesosEstimados` memo no lugar do cálculo por célula; `mediaMonte` da sugestão também inteira); `Intl.NumberFormat` com `maximumFractionDigits` já omite `,00` para inteiros; PRD RF-P01/RF-P03 atualizados com a fórmula e a compensação
- [x] 2026-09-19 — **Unificação visual dos layouts (fluxo autenticado)**: dashboard e relatórios migraram do header `sticky` custom para o **`ios-topbar`** padrão (com Sino + ToggleTema) e o **chip "Menu" foi removido** (navegação já coberta pela aba "Mais" da TabBar); **configurações ganhou `ios-topbar` (Sino + Tema, "← Menu" removido) e `<TabBar />`** — antes era a única tela autenticada sem navegação inferior; **largura global unificada em `max-w-[480px]`**: TabBar (`max-w-3xl`→480) e `.ios-topbar` centralizado em 480 (mesma geometria da TabBar) — header, conteúdo e barra inferior alinhados na mesma coluna em todas as telas
